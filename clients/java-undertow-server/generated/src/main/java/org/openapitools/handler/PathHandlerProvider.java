    ...
    * retain any state between multiple sessions.</p>
    ...
    public HttpHandler getHandler(final String basePath) {
        return Handlers.routing()
            .add(Methods.GET, basePath + "/api/all.json", new HttpHandler() {
                @Override
                public void handleRequest(HttpServerExchange exchange) throws Exception {
                    getApiAllJson().handleRequest(exchange);
                }
            })
            .add(Methods.GET, basePath + "/api/{product}/{cycle}.json", new HttpHandler() {
                @Override
                public void handleRequest(HttpServerExchange exchange) throws Exception {
                    getApiProductCycleJson().handleRequest(exchange);
                }
            })
            .add(Methods.GET, basePath + "/api/{product}.json", new HttpHandler() {
                @Override
                public void handleRequest(HttpServerExchange exchange) throws Exception {
                    getApiProductJson().handleRequest(exchange);
                }
            })
            ;
    }
    ...
    public HttpHandler getStatefulHandler(final String basePath) {
        return Handlers.routing()
            .add(Methods.GET, basePath + "/api/all.json", getApiAllJson())
            .add(Methods.GET, basePath + "/api/{product}/{cycle}.json", getApiProductCycleJson())
            .add(Methods.GET, basePath + "/api/{product}.json", getApiProductJson())
            ;
    }
}
