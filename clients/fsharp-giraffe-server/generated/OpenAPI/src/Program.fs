...
module App =
  ...
  let webApp =
    choose (CustomHandlers.handlers @ [
      HttpGet >=> route "/api/all.json" >=>  DefaultApiHandler.GetApiAllJson;
      HttpGet >=> routeBind<GetApiProductCycleJsonPathParams> "/api/{product}/{cycle}.json"  (fun x -> (fun x ->  DefaultApiHandler.GetApiProductCycleJson x) x);
      HttpGet >=> routeBind<GetApiProductJsonPathParams> "/api/{product}.json"  (fun x ->  DefaultApiHandler.GetApiProductJson x);
