    ...
    B::Error: Into<Box<dyn Error + Send + Sync>>,
{
        ...
        context: &C) -> Result<GetApiProductJsonResponse, ApiError>
    {
        ...
        let mut uri = format!(
            "{}/api/{product}.json",
            self.base_path
            ,product=utf8_percent_encode(&param_product.to_string(), ID_ENCODE_SET)
        );
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {e}")))?;
                let body = serde_json::from_str::<Vec<models::Cycle>>(body)
                    .map_err(|e| ApiError(format!("Response body did not match the schema: {e}")))?;


                Ok(GetApiProductJsonResponse::OK
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<GetApiProductCycleJsonResponse, ApiError>
    {
        ...
        let mut uri = format!(
            ...
            self.base_path
            ,product=utf8_percent_encode(&param_product.to_string(), ID_ENCODE_SET)
            ,cycle=utf8_percent_encode(&param_cycle.to_string(), ID_ENCODE_SET)
        );
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {e}")))?;
                let body = serde_json::from_str::<models::Cycle>(body)
                    .map_err(|e| ApiError(format!("Response body did not match the schema: {e}")))?;


                Ok(GetApiProductCycleJsonResponse::OK
                    (body)
                )
            }
            code => {
