...
public class CycleSupport extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(CycleSupport.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!CycleSupport.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'CycleSupport' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<CycleSupport>() {
                @Override
                public void write(JsonWriter out, CycleSupport value) throws IOException {
                ...
                @Override
                public CycleSupport read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        CycleSupport ret = new CycleSupport();
                    ...
                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for CycleSupport: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to CycleSupport
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for CycleSupport with anyOf schemas: Boolean, String. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of CycleSupport given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of CycleSupport
     * @throws IOException if the JSON string is invalid with respect to CycleSupport
     */
    public static CycleSupport fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, CycleSupport.class);
    }

    /**
     * Convert an instance of CycleSupport to an JSON string
     *
     * @return JSON string
     */
