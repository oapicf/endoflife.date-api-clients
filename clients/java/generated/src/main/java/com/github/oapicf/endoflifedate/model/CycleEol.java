...
public class CycleEol extends AbstractOpenApiSchema {
    private static final Logger log = Logger.getLogger(CycleEol.class.getName());

    public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
        @SuppressWarnings("unchecked")
        @Override
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            if (!CycleEol.class.isAssignableFrom(type.getRawType())) {
                return null; // this class only serializes 'CycleEol' and its subtypes
            }
            ...
            return (TypeAdapter<T>) new TypeAdapter<CycleEol>() {
                @Override
                public void write(JsonWriter out, CycleEol value) throws IOException {
                ...
                @Override
                public CycleEol read(JsonReader in) throws IOException {
                    ...
                    throw new IOException(String.format(java.util.Locale.ROOT, "Failed deserialization for CycleEol: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
     ...
     * @throws IOException if the JSON Element is invalid with respect to CycleEol
     */
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format(java.util.Locale.ROOT, "The JSON string is invalid for CycleEol with anyOf schemas: Boolean, String. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }

    /**
     * Create an instance of CycleEol given an JSON string
     *
     * @param jsonString JSON string
     * @return An instance of CycleEol
     * @throws IOException if the JSON string is invalid with respect to CycleEol
     */
    public static CycleEol fromJson(String jsonString) throws IOException {
        return JSON.getGson().fromJson(jsonString, CycleEol.class);
    }

    /**
     * Convert an instance of CycleEol to an JSON string
     *
     * @return JSON string
     */
